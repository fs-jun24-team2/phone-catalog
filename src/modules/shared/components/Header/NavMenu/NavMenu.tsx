...
export const NavMenu: React.FC<NavMenuProps> = ({ language, isMenuOpen, setIsMenuOpen }) => {
  return (
    <nav className={`${styles.menu} ${isMenuOpen ? styles.menu__open : ''}`}>
      <ul className={styles.menu__list}>
        <li className={styles.menu__item}>
          <NavLink
            to={Path.main}
            className={({ isActive }) => (isActive ? `${styles.menu__link} ${styles.is_active}` : styles.menu__link)}
